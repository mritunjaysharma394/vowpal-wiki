...
`prediction` is VW's prediction on the example with tag `tag`. Then there's a list of feature information starting with the namespace, the name of the feature, the position where it hashes, the value of the feature, the current learned weight associated with that feature and finally the sum of squared gradients (plus 1) if adaptive updates are used.
